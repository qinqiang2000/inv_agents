...
# 进度显示配置
PROGRESS_INTERVAL="${PROGRESS_INTERVAL:-10}"  # 每处理多少条记录显示一次进度
DETAILED_PROGRESS="${DETAILED_PROGRESS:-true}" # 是否显示详细进度信息
SHOW_FIRST_RECORDS="${SHOW_FIRST_RECORDS:-5}"  # 前几条记录显示详细信息
...
log_progress() {
    ...
    if [ "${DETAILED_PROGRESS}" = "true" ]; then
        echo -e "${CYAN}[PROGRESS]${NC} 记录 #${count}: 租户${tenant} ${country} ${invoice_no}"
    else
        echo -e "${CYAN}[PROGRESS]${NC} 已处理 ${count} 条记录"
    fi
}
...
init_state_file() {
    ...
    if [ -s "${STATE_FILE}" ]; then
        if ! grep -qE '^[0-9]+\|[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]+\|[0-9]+\|[A-Z]+$' "${STATE_FILE}"; then
...
export_full() {
    log_info "=========================================="
    log_info "执行全量导出"
    log_info "进度间隔: ${PROGRESS_INTERVAL} 条记录"
    log_info "详细进度: ${DETAILED_PROGRESS}"
    log_info "=========================================="
    ...
    while IFS=$'\t' read -r tenant_id country issue_date invoice_no ext_field update_time; do
        ...
        if process_invoice_record "${tenant_id}" "${country}" "${issue_date}" "${invoice_no}" "${ext_field}"; then
            ...
            if [ ${export_count} -le ${SHOW_FIRST_RECORDS} ]; then
                show_detailed=true
            elif [ $((export_count % PROGRESS_INTERVAL)) -eq 0 ]; then
                show_detailed=true
            fi
            ...
            if [ "$show_detailed" = "true" ]; then
                ...
                if [ "${DETAILED_PROGRESS}" = "true" ]; then
                    # 显示详细进度信息
                    echo -e "${MAGENTA}[PROGRESS]${NC} #${export_count}: 租户${tenant_id} ${country} ${invoice_no} (耗时: ${elapsed}s, 总计: ${total_elapsed}s)"
                else
                    # 简化进度信息
                    if [ $((export_count % (PROGRESS_INTERVAL * 10))) -eq 0 ]; then
                        log_info "已导出 ${export_count} 条记录... (总耗时: ${total_elapsed}s)"
                    else
                        echo -e "${CYAN}[PROGRESS]${NC} 已导出 ${export_count} 条记录"
                    fi
                fi
            ...
            ((error_count++))
            log_error "处理失败: 租户${tenant_id} 发票号${invoice_no}"
        fi

        # 每处理1000条记录显示内存使用情况（可选）
        if [ $((export_count % 1000)) -eq 0 ] && [ ${export_count} -gt 0 ]; then
            if command -v ps &> /dev/null; then
                local memory_usage=$(ps -o rss= -p $$ | awk '{print $1}')
                if [ -n "$memory_usage" ]; then
                    echo -e "${CYAN}[MEMORY]${NC} 进程内存使用: $((memory_usage / 1024))MB"
                fi
            fi
        fi
    done
    ...
    if [ ${export_count} -gt 0 ]; then
        local avg_time=$(echo "scale=3; ${total_duration}/${export_count}" | bc -l 2>/dev/null || echo "N/A")
        log_info "平均每条记录处理时间: ${avg_time} 秒"
    fi
...
export_incremental() {
    ...
    for tenant_id in ${tenants}; do
        ...
        local tenant_start_time=$(date +%s)
        ...
        while IFS=$'\t' read -r tid country issue_date invoice_no ext_field update_time; do
            ...
            if process_invoice_record "${tid}" "${country}" "${issue_date}" "${invoice_no}" "${ext_field}"; then
                ...
                # 增量模式进度显示
                if [ $((tenant_export_count % PROGRESS_INTERVAL)) -eq 0 ] || [ ${tenant_export_count} -le ${SHOW_FIRST_RECORDS} ]; then
                    echo -e "${CYAN}[PROGRESS]${NC} 租户${tid}: 已导出 ${tenant_export_count} 条新记录"
                fi
            else
                ((tenant_error_count++))
            fi
        ...
        local tenant_end_time=$(date +%s)
        local tenant_duration=$((tenant_end_time - tenant_start_time))

        if [ ${tenant_export_count} -gt 0 ]; then
            log_success "租户 ${tenant_id}: 导出 ${tenant_export_count} 条新记录 (耗时: ${tenant_duration}s)"
        else
            log_info "租户 ${tenant_id}: 无新增数据 (耗时: ${tenant_duration}s)"
        fi
    done
    ...
    log_info "总耗时: ${total_duration} 秒"
    if [ ${total_export_count} -gt 0 ]; then
        local avg_time=$(echo "scale=3; ${total_duration}/${total_export_count}" | bc -l 2>/dev/null || echo "N/A")
        log_info "平均每条记录处理时间: ${avg_time} 秒"
    fi
    log_info "输出目录: ${OUTPUT_BASE_DIR}"
...
main() {
    # 显示配置信息
    log_info "发票数据导出脚本启动"
    log_info "进度显示配置: 每${PROGRESS_INTERVAL}条记录显示进度"
    log_info "详细模式: ${DETAILED_PROGRESS}"
    if [ "${DETAILED_PROGRESS}" = "false" ]; then
        log_info "提示: 设置 DETAILED_PROGRESS=true 可显示更详细的进度信息"
    fi
